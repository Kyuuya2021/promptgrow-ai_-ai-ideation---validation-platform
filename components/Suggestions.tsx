...
const Suggestions: React.FC<SuggestionsProps> = ({ suggestions, isLoading, onSuggestionClick }) => {
  ...
  return (
    <div className="mb-3 max-w-full">
        ...
        {isLoading ? (
          <div className="bg-[var(--bg-muted)] text-[var(--text-muted)] text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full">
            <LoadingBubble />
          </div>
        ) : (
          suggestions.map((suggestion, index) => (
            <button
              ...
              className="bg-[var(--bg-muted)] text-[var(--text-muted)] text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full hover:bg-[var(--bg-hover)] hover:text-[var(--text-base)] transition-colors text-left transform active:scale-95 touch-manipulation"
            >
              {suggestion}
            </button>
          ))
