...
const Welcome: React.FC<{ mode: Mode }> = ({ mode }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-4 text-[var(--text-muted)] overflow-y-auto">
      ...
      <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-2 sm:mb-3 text-[var(--text-base)] tracking-tight px-4">{t('welcome.title')}</h1>
      <p className="mb-6 sm:mb-8 md:mb-12 max-w-2xl text-sm sm:text-base md:text-lg px-4">{t('welcome.subtitle')}</p>
      
      <div className="w-full max-w-5xl text-left px-4">
        <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-[var(--text-base)]">{t('welcome.modesOverview')}</h2>
        <div className="grid grid-cols-1 gap-3 sm:gap-4">
          {Object.values(Mode).map(m => {
            ...
            return (
              <div 
                key={m} 
                className={`p-4 sm:p-5 rounded-xl sm:rounded-2xl border-2 transition-all duration-300 transform hover:-translate-y-1 ${
                  isCurrent 
              ...
              >
                <div className="flex items-start mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-base sm:text-lg text-[var(--text-base)] mb-2">{t(`modes.${m}.name`)}</h3>
                    <p className="text-sm text-[var(--text-muted)] leading-relaxed break-words">{t(`modes.${m}.description`)}</p>
                  </div>
                </div>
              </div>
            );
